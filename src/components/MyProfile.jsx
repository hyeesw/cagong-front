...
function MyProfile({ userInfo }) {
  ...
  if (userInfo) {
    ...
    SmallProfilePhoto = (
      <div
        className="circle"
        style={{ width: '50px', height: '50px', borderRadius: '50%', backgroundColor: 'lightgrey' }}
      />
    );
    BigProfilePhoto = (
      <div
  ...
  return (
    <>
      <Dropdown align="end" style={{ position: 'relative' }}>
        <Dropdown.Toggle as={CustomToggle} id="dropdown-custom-components" userInfo={userInfo}>
          {SmallProfilePhoto}
        </Dropdown.Toggle>

        <Dropdown.Menu
          align="end"
          style={{ minWidth: '400px', padding: '20px', position: 'absolute', right: 0 }}
        >
          {userInfo ? (
            // 로그인 O
            <>
              ...
              >
                {userInfo.username}
                {/* 로그인 후, localStorage에 저장되어있던 userInfo에서, 'username'의 value값 */}
              </Dropdown.Item>
